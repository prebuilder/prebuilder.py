...
from .core.Package import AssociateTearingSpecAndMetadata, BasePackageRef, Package, PackageInstalledFiles, PackageMetadata, PackageRef, PackagingSpec, VersionedPackageRef
...
from .extractors.ELF import ELFDepsResolver, ELFExtractor, _ELFDepsResolver
...
class DistroStream:
	...
	def _tearPackage(self, tr: AssociateTearingSpecAndMetadata):
		...
		print(styles.entity("torn") + " " + rsjoin(", ", (styles.varContent(str(individualMetadata if isinstance(individualMetadata, VersionedPackageRef) else individualMetadata.ref)) for individualMetadata, tearingSpec in tr.packagingSpec.tearingSpecs)))
